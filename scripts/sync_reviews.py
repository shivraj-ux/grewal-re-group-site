...
def other_platform_count(current: dict) -> int:
    ...
    The site shows the all-platform total (primary Google + these) in the
    hero/proof stats and schema, so the total must move together with the
    primary Google count on every sync.

    RateMyAgent was evaluated and deliberately excluded (see reviews.json ->
    checked_no_additional_reviews.ratemyagent_excluded_2026-07-11) -- a large
    share of its reviews duplicate clients already counted under Google, so
    it does not belong in this sum.
    """
